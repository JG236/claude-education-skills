...
output_schema:
  ...
  - field: gap_analysis
    type: object
    description: Specific gaps identified, classified by type — conceptual, procedural,
      or communication
  - field: strengths
    type: array
  ...
  - field: next_teaching_steps
    type: array
    description: Targeted actions to address each gap — not generic advice but specific
      next steps
  - field: feedback_script
    type: string
    description: How to communicate the analysis to the student in a way that promotes
      improvement
chains_well_with:
